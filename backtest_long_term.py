...
def load_data_feed(symbol, is_crypto):
    ...
    if is_crypto:
        ...
        try:
            ...
            df_list = []
            cur = now_ms - (1825 * 24 * 3600 * 1000)
            while cur < now_ms and len(df_list) < 18:
                b_df = get_binance_klines(symbol=bin_sym, interval="1d", limit=1000, start_time=cur)
                if b_df.empty:
                    break
                df_list.append(b_df)
                cur = int(b_df["timestamps"].iloc[-1].timestamp() * 1000) + 86400000
                time.sleep(0.05)
            if df_list:
...
def load_vix_data():
    ...
    try:
        vix = yf.Ticker("^VIX")
        df = vix.history(period="5y", interval="1d")
        df.reset_index(inplace=True)
        for col in ['Datetime', 'Date']:
            if col in df.columns:
                df.rename(columns={col: 'timestamps'}, inplace=True)
